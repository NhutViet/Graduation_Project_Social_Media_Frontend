...
import {updateIsFollowPostUser} from '@services/postUserRedux/postUserReducer';
import { updateTaggedPostFollow } from '@services/taggedPostRedux/taggedPostReducer';
import { updateFollowStatusForUser } from '@services/bookmarkRedux/bookmarkReducer';
...
>(
  ...
  async (
    {targetId, action, senderId, handleName},
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        if (action === 'follow' || action === 'unfollow') {
          ...
          dispatch(
            updateIsFollowPostUser({payload: {
              userId: targetId,
              isFollow: action === 'follow',
            }}),
          );
          dispatch(
            updateTaggedPostFollow({
              userId: targetId,
              isFollow: action === 'follow',
            }),
          );
...
export const fetchViewedFollowers = createAsyncThunk<
  ...
  {userId: string},
  {rejectValue: string}
>('relations/viewedFollowers', async ({userId}, {rejectWithValue}) => {
  try {
...
export const fetchViewedFollowing = createAsyncThunk<
  ...
  {userId: string},
  {rejectValue: string}
>('relations/viewedFollowing', async ({userId}, {rejectWithValue}) => {
  try {
    const res = await axiosInstance.post(
      API.GET_FOLLOWING,
      {userId},
