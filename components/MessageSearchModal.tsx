...
export const MessageSearchModal: React.FC<MessageSearchModalProps> = memo(
  ({visible, onClose, messages, onMessageSelect}) => {
    ...
    const performSearch = useCallback(
      (query: string) => {
        ...
        messages.forEach((message, index) => {
          const isTextMessage =
            message.content && !message.media && message.content.trim() !== '' && message.content.trim() !== 'Tin nhắn đã bị thu hồi';
    ...
    const renderHighlightedText = useCallback(
      (text: string) => {
        ...
        return (
          <Text style={[styles.messageContent, {color: color.textSecondary}]} numberOfLines={1}>
            {parts.map((part, index) => {
              if (part.startsWith('**') && part.endsWith('**')) {
                return (
                  <Text key={index} style={[styles.highlightedText, {color: color.text}]}>
                    {part.slice(2, -2)}
                  </Text>
                );
              }
    ...
    const renderSearchResult = useCallback(
      ({item, index}: {item: SearchResult; index: number}) => (
        <TouchableOpacity
          ...
          <Image
            source={{uri: item.message.sender.profilePic}}
            style={styles.UserImg}
          />
          <View style={[styles.resultContent, {flex: 1}]}>
            <View style={styles.messageHeader}>
              <Text style={[styles.senderName, {color: color.text}]}>
    ...
    const keyExtractor = useCallback(
      (item: SearchResult, index: number) => `${item.message._id}-${index}`,
      [],
    );
          ...
          <View style={[styles.header]}>
            <View style={styles.searchContainer}>
              ...
              <TouchableOpacity onPress={onClose} style={styles.cancelButton}>
                <Text style={[styles.cancelText, {color: color.primary}]}>
                  Hủy
                </Text>
              </TouchableOpacity>
            </View>
